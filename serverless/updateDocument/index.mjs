import { S3Client, PutObjectCommand, DeleteObjectCommand } from "@aws-sdk/client-s3";
import { DynamoDBClient } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, UpdateCommand } from "@aws-sdk/lib-dynamodb";
import { PassThrough } from "stream";
import Busboy from "busboy";
import { Buffer } from "buffer";
import { v4 as uuidv4 } from "uuid";
import { verifyTokenFromHeader } from "/opt/verifyToken/index.mjs";

const s3 = new S3Client({ region: "us-east-2" });
const dbClient = new DynamoDBClient({ region: "us-east-2" });
const ddb = DynamoDBDocumentClient.from(dbClient);

export const handler = async (event) => {
  return new Promise(async (resolve) => {
    try {
      const authHeader = event.headers?.Authorization || event.headers?.authorization;
      await verifyTokenFromHeader(authHeader); 

      const contentType = event.headers["content-type"] || event.headers["Content-Type"];
      const busboy = Busboy({ headers: { "content-type": contentType } });

      let fileBuffer = Buffer.alloc(0);
      let fileName, mimeType;
      let userId, typeId, typeName, documentId, oldS3Key;

      busboy.on("file", (fieldname, file, filename, encoding, mimetype) => {
        if (typeof filename === "object" && filename?.filename) {
          fileName = filename.filename;
        } else {
          fileName = filename;
        }

        mimeType = mimetype;

        file.on("data", (data) => {
          fileBuffer = Buffer.concat([fileBuffer, data]);
        });
      });

      busboy.on("field", (fieldname, val) => {
        if (fieldname === "userId") userId = val;
        if (fieldname === "typeId") typeId = val;
        if (fieldname === "typeName") typeName = val;
        if (fieldname === "documentId") documentId = val;
        if (fieldname === "s3Key") oldS3Key = val;
      });

      busboy.on("finish", async () => {
        if (!fileName || !userId || !typeId || !typeName || !documentId || !oldS3Key) {
          return resolve({
            statusCode: 400,
            body: JSON.stringify({ message: "Missing required fields" }),
          });
        }

        await s3.send(new DeleteObjectCommand({
          Bucket: process.env.AWS_BUCKET_NAME,
          Key: oldS3Key,
        }));

        const actualFileName = typeof fileName === "string"
        ? fileName
        : String(fileName?.toString?.() || "file");

        const cleanFileName = actualFileName
          .replace(/\s+/g, "_")
          .replace(/[^a-zA-Z0-9._-]/g, "");

        const finalFileName = `${uuidv4()}-${cleanFileName}`;

        await s3.send(new PutObjectCommand({
          Bucket: process.env.AWS_BUCKET_NAME,
          Key: finalFileName,
          Body: fileBuffer,
          ContentType: mimeType,
        }));

        const updateParams = {
          TableName: process.env.USER_DOCUMENTS_TABLE,
          Key: { userId, documentId },
          UpdateExpression: `
            SET typeId = :typeId,
                typeName = :typeName,
                s3Url = :s3Url,
                s3Key = :s3Key,
                uploadedAt = :uploadedAt
          `,
          ExpressionAttributeValues: {
            ":typeId": typeId,
            ":typeName": typeName,
            ":s3Url": `https://${process.env.AWS_BUCKET_NAME}.s3.amazonaws.com/${finalFileName}`,
            ":s3Key": finalFileName,
            ":uploadedAt": new Date().toISOString(),
          },
        };

        await ddb.send(new UpdateCommand(updateParams));

        return resolve({
          statusCode: 200,
          body: JSON.stringify({
            message: "Document updated successfully",
            document: {
              userId,
              documentId,
              typeId,
              typeName,
              s3Url: `https://${process.env.AWS_BUCKET_NAME}.s3.amazonaws.com/${finalFileName}`,
              s3Key: finalFileName,
            },
          }),
        });
      });

      const body = Buffer.from(event.body, event.isBase64Encoded ? "base64" : "utf8");
      const stream = new PassThrough();
      stream.end(body);
      stream.pipe(busboy);
    } catch (err) {
      console.error("Error updating document:", err);
      return resolve({
        statusCode: 500,
        body: JSON.stringify({ message: "Update failed", error: err.message }),
      });
    }
  });
};